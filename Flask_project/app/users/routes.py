...
from app.users.forms import SetPasswordForm, UserEditForm, UserRegistrationForm
...
def user_list():
    ...
    next_url = (
        url_for("users.user_list", page=users.next_num) if users.has_next else None
    )
    prev_url = (
        url_for("users.user_list", page=users.prev_num) if users.has_prev else None
    )
