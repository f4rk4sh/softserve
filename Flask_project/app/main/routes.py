...
def question_list():
    ...
    next_url = (
        url_for("main.question_list", page=questions.next_num)
        if questions.has_next
        else None
    )
    prev_url = (
        url_for("main.question_list", page=questions.prev_num)
        if questions.has_prev
        else None
    )
    return render_template(
        "main/question_list.html",
        title="Questions",
...
def interview_list():
    page = request.args.get("page", 1, type=int)
    interviews = (
        Interview.query.filter(Interview.date >= date.today())
        .order_by("date")
        .paginate(page, 8, False)
    )
    next_url = (
        url_for("main.interview_list", page=interviews.next_num)
        if interviews.has_next
        else None
    )
    prev_url = (
        url_for("main.interview_list", page=interviews.prev_num)
        if interviews.has_prev
        else None
    )
    return render_template(
        "main/interview_list.html",
        title="Interviews",
...
def interview_list_archive():
    ...
    next_url = (
        url_for("main.interview_list_archive", page=interviews.next_num)
        if interviews.has_next
        else None
    )
    prev_url = (
        url_for("main.interview_list_archive", page=interviews.prev_num)
        if interviews.has_prev
        else None
    )
    return render_template(
        "main/interview_list_archive.html",
        title="Archive",
        interviews=interviews.items,
        next_url=next_url,
        prev_url=prev_url,
...
def interview_add():
    ...
    form.users.query = db.session.query(User).filter_by(
        is_admin=False, is_recruiter=False
    )
    if form.validate_on_submit():
...
def interview_detail(interview_id):
    ...
    rated = (
        ...
        if Grade.query.filter_by(
            user_id=current_user.id, interview_id=interview.id
        ).first()
        else False
    )
    return render_template(
        "main/interview_detail.html",
...
def activities():
    ...
    interviews = (
        Interview.query.filter(
            Interview.users.contains(current_user), Interview.date >= date.today()
        )
        .order_by("date")
        .paginate(page, 8, False)
    )
    next_url = (
        url_for("main.activities", page=interviews.next_num)
        if interviews.has_next
        else None
    )
    prev_url = (
        url_for("main.activities", page=interviews.prev_num)
        if interviews.has_prev
        else None
    )
    return render_template(
...
def interview(interview_id):
    ...
    if grade_set_form.validate_on_submit():
        ...
        interview.score = (
            ((interview.score / 100 * max_score * len(interview.users)) + score)
            / len(interview.users)
            / max_score
            * 100
        )
