...
class QuestionForm(FlaskForm):
    ...
    max_grade = IntegerField(
        "Max grade", validators=[DataRequired(), NumberRange(min=1, max=5)]
    )
...
class QuestionSetForm(FlaskForm):
    ...
    level = SelectField(
        "Level",
        choices=["trainee", "junior", "middle", "senior", "tech lead", "team lead"],
...
class RecruiterInterviewForm(FlaskForm):
    interviewee = StringField(
        "Interviewee", validators=[DataRequired(), length(max=50)]
    )
